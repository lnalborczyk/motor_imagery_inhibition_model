...
loss_function <- function (
    ...
    # the relative imagery threshold can be learned as well
    ...
    peak_time_inhib <- peak_time_activ
    curvature_inhib <- par[[4]] * curvature_activ
        ...
        fit <- DEoptim::DEoptim(
            ...
            control = DEoptim.control(
                ...
                initialpop = as.matrix(data.frame(
                    ...
                    lhs(n = 200, rect = c(-0.6, 0.4) )[, 1],
                    lhs(n = 200, rect = c(0.1, 0.5) )[, 1],
                    lhs(n = 200, rect = c(1.1, 2) )[, 1]
                    ) ),
                # using all available cores
...
lhs_pars <- data.frame(
    ...
    lhs(n = nstudies, rect = c(-0.6, 0.4) )[, 1],
    lhs(n = nstudies, rect = c(0.1, 0.5) )[, 1],
    lhs(n = nstudies, rect = c(1.1, 2) )[, 1]
    )
...
predicted_rt_mt <- lhs_pars %>%
    ...
    do(
        suppressWarnings(
            balance_function(
                amplitude_activ = 1.5,
                peak_time_activ = .$peak_time,
                curvature_activ = .$curvature_activ,
                amplitude_inhib = 1.5 / .$amplitude_ratio,
                peak_time_inhib = .$peak_time,
...
save(
    par_recov_results,
    file = "4pars_200obs_500_sims_DEoptim_1000iter_g2_lhs.Rdata"
    )
