...
# This time varying peak times (4 pars)      #
# ------------------------------------------ #
    ...
    mutate(
        balance_end_of_trial = (amplitude_ratio) *
            exp(-(log(3) - log(peak_time_activ) )^2 / (2 * (1 / sqrt(6) )^2) +
                    (log(3) - log(peak_time_inhib * peak_time_activ) )^2 / (2 * (curvature_inhib * 1 / sqrt(6) )^2) )
        ) %>%
    mutate(
        included = case_when(
...
for (i in 1:max(par_recov_results$study_id) ) {
    ...
    temp_df <- model(
        ...
        curvature_inhib = true_pars[4] * (1 / sqrt(6) )
        ) %>%
...
save(
    par_recov_results,
    file = "4pars_v2_50to500obs_500sims_DEoptim_1000iter_g2_lhs.Rdata"
    )
